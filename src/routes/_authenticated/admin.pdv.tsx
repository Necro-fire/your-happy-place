  ...
  return (
    <div className="flex flex-col gap-4 lg:h-[calc(100dvh-7rem)]">
      ...
      {attendModalOpen && atendimento && atendimento !== "balcao" && (
        <Dialog open onOpenChange={(o) => { if (!o) cancelAttendModal(); }}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>
                Dados do atendimento — {ATENDIMENTOS.find((a) => a.key === atendimento)?.label}
              </DialogTitle>
            </DialogHeader>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={cancelAttendModal}>Cancelar</Button>
              <Button onClick={() => setAttendModalOpen(false)}>Confirmar</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
